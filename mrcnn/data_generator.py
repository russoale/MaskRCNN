...
class DataGenerator(KU.Sequence):
    """A generator that returns images and corresponding target class ids,
        ...
        shuffle: If True, shuffles the sample-to-batch assignment
                Dataset wide shuffling (on batch level) is already handled by Keras.
        process_unique_shuffle: If True, use PID-based shuffling
                otherwise shuffle identical across all processes. (Intended for multiprocessing)
        ...
        num_workers: (Optional) Helps to determine when a real epoch is over.
                     Is used to shuffle at the end of an epoch (when enabled).
    ...
    def __init__(self, dataset, config, shuffle=True, process_unique_shuffle=False, augmentation=None,
                 random_rois=0, batch_size=1, detection_targets=False, no_augmentation_sources=None):
        self.dataset = dataset
        self.config = config
        self.shuffle = shuffle
        self.process_unique_shuffle = process_unique_shuffle
        ...
        self.backbone_shapes = compute_backbone_shapes(self.config, self.config.IMAGE_SHAPE)
    ...
    def __len__(self):
        return int(np.floor(len(self.image_ids) / float(self.batch_size)))

    def on_epoch_end(self):
        # Gets called at the end of a "real" epoch
        # If Shuffling is enabled, reshuffle
        if self.shuffle:
            np.random.shuffle(self.image_ids)
    ...
    def __getitem__(self, idx):
        if self.pid != os.getpid():
            ...
            self.pid = os.getpid()
            if self.shuffle:
                # Initial shuffle (in the multiprocessing case)
                if self.process_unique_shuffle:
                    np.random.seed(self.parent_pid)
                np.random.shuffle(self.image_ids)
        ...
        # Run until we have a complete batch
        while b < self.batch_size:
            current_image_index = image_index
            # Increment image_index for next loop run
            image_index = (image_index + 1) % len(self.image_ids)
            try:

                # Get GT bounding boxes, masks and keypoints for image.
                image_id = self.image_ids[current_image_index]
                ...
                # Mask R-CNN Targets
                if self.random_rois:
                    rpn_rois = self.generate_random_rois(image.shape, gt_boxes)
                    if self.detection_targets:
                        rois, mrcnn_class_ids, mrcnn_bbox, mrcnn_mask, mrcnn_keypoints, mrcnn_keypoint_weights = \
                            self.build_detection_targets(rpn_rois, gt_class_ids, gt_boxes, gt_masks, gt_keypoints)
    ...
    def build_detection_targets(self, rpn_rois, gt_class_ids, gt_boxes, gt_masks, gt_keypoints):
    ...
    def load_image_gt(self, image_id, augmentation=None, use_mini_mask=False):
    ...
    def build_rpn_targets(self, gt_class_ids, gt_boxes):
        ...
        if crowd_ix.shape[0] > 0:
            ...
            crowd_overlaps = utils.compute_overlaps(self.anchors, crowd_boxes)
            crowd_iou_max = np.amax(crowd_overlaps, axis=1)
            no_crowd_bool = (crowd_iou_max < 0.001)
        else:
            # All anchors don't intersect a crowd
            no_crowd_bool = np.ones([self.anchors.shape[0]], dtype=bool)

        # Compute overlaps [num_anchors, num_gt_boxes]
        overlaps = utils.compute_overlaps(self.anchors, gt_boxes)
        ...
        for i, a in zip(ids, self.anchors[ids]):
    ...
    def generate_random_rois(self, image_shape, gt_boxes):
