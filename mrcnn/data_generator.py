...
def load_image_gt(dataset, config, image_id, augmentation=None, use_mini_mask=False):
    ...
    keypoints, mask, class_ids = dataset.load_mask_keypoints(image_id)
        ...
        while b < self.batch_size:
            ...
            if self.dataset.image_info[image_id]['source'] in self.no_augmentation_sources:
                image, image_meta, gt_class_ids, gt_boxes, gt_masks, gt_keypoints = \
                    load_image_gt(self.dataset, self.config, image_id, augmentation=None,
                                  use_mini_mask=self.config.USE_MINI_MASK)
            else:
                image, image_meta, gt_class_ids, gt_boxes, gt_masks, gt_keypoints = \
                    load_image_gt(self.dataset, self.config, image_id, augmentation=self.augmentation,
                                  use_mini_mask=self.config.USE_MINI_MASK)
            ...
            if b == 0:
                ...
                batch_gt_keypoints = np.zeros(
                    (self.batch_size, self.config.MAX_GT_INSTANCES, Num_keypoint, 3))
